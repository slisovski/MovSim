...
rot <- cbind(seq(0,  18, length = nrow(TIFFindex)), 
             seq(125, 130, length = nrow(TIFFindex)))
...
for(i in 1:nrow(TIFFindex)) {
  ...
  #     path(x = 0, y = 7, material = diffuse(color="grey80"), width = 0.002, angle=c(180,rot[i,2],0))
  # }, mc.cores = 4))
  # 
  
  ### Bar-tailed godwit tracks
  realTracks <- subset(interpTracksWind, tms<=TIFFindex[i,1])
  ...
  grPoints <- dplyr::bind_rows(parallel::mclapply(split(as.data.frame(cbind(st_coordinates(t_real)[,1:2], id = 1:nrow(st_coordinates(t_real)), colID = do.call("rbind", last)$colID)), 
          ...
          function(x) {
              sp <- sph2car(matrix(c((x$X+rot[i,2])*-1, x$Y, 1), ncol = 3))[,c(1,3,2)]
  ...
  generate_studio(material=diffuse(color = "grey10")) %>%
    add_object(sphere(x = 0, y = 7, radius=0.9999, angle = c(0,rot[i,2],0),
    ...
    render_scene(width=250, height=250, aperture=0, fov=14, sample_method = "random", parallel = TRUE,
                 samples= 100, clamp_value=10, lookfrom=c(1,7,10), lookat=c(0,7,0), camera_up = c(0,1,0), filename=glue::glue("~/Desktop/tmp/globe_{i}.png"))
  
  
  title_mat = matrix(0,60,250) %>%
    add_title(title_text = glue::glue("{format(TIFFindex[i,1], '%Y-%m-%d')}"), 
